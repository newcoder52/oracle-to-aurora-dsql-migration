...
|-- security/
|   |-- s3-bucket-policy.json          # S3 bucket policy enforcing TLS
```
...
2. An S3 bucket for intermediate data storage with Block Public Access enabled and default encryption (SSE-S3 or SSE-KMS) configured
...
- **Input Validation**: Table names are validated against a strict regex pattern (`^[a-zA-Z0-9_]{1,64}$`) to prevent SQL injection
- **Least-Privilege IAM**: All IAM policies use scoped resource ARNs (no wildcards). Permissions are limited to only the specific actions required
- **S3 Security**: Block Public Access enabled, default encryption (SSE-KMS), and a bucket policy enforcing TLS for all data transfers (see `security/s3-bucket-policy.json`)
...
- **Short-Lived Tokens**: Aurora DSQL auth tokens expire after 1 hour (ExpiresIn=3600)
